/**
 * MainAllGamesPage Component
 *
 * Displays all upcoming and previous games for the user
 *
 * Features:
 * - Tabs for Upcoming and Previous games
 * - List of all games with vertical scrolling
...
import { previousGames, upcomingGames } from './mockData';

interface MainAllGamesPageProps {
  // Future: Add props for games data when connected to real API
}

type TabType = 'upcoming' | 'previous';

export const MainAllGamesPage: React.FC<MainAllGamesPageProps> = () => {
  ...
  const handleGamePress = (gameId: string) => {
    // Find the game to get its sport and determine if it's upcoming or previous
    const upcomingGame = upcomingGames.find(g => g.id === gameId);
    const previousGame = previousGames.find(g => g.id === gameId);
    const game = upcomingGame || previousGame;

    const sport = game?.sport || 'soccer';
    const isUpcoming = !!upcomingGame;

    // Navigate to match details page with sport and status parameters
    if (isUpcoming) {
      router.push(`/match?sport=${sport}&useUpcoming=true`);
    } else {
      router.push(`/match?sport=${sport}`);
    }
    console.log('Game pressed:', gameId, 'sport:', sport, 'upcoming:', isUpcoming);
  };

  // Get unique teams, sports, and tournaments from data
  ...
  return (
    <View style={styles.container}>
      ...
      <GamesListWithTabs
        ...
        tabTextActiveColor={COLORS.white}
...
export default MainAllGamesPage;
