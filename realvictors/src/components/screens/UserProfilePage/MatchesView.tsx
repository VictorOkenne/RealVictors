 ...
 * Uses the GamesListWithTabs component for reusability
 * Features smart filtering with default sport-based filtering
 */
...
import { SportType } from './mockData';
...
export const MatchesView: React.FC<MatchesViewProps> = ({ currentSport = 'soccer' }) => {
  ...
  // Initialize filters with current sport as default
  const [filters, setFilters] = React.useState<FilterState>({
    sortBy: 'recent',
    result: [],
    sport: [currentSport], // Default to current sport
    team: [],
    tournament: [],
    dateRange: 'allTime',
  });
  ...
  // Update default sport filter when currentSport prop changes
  React.useEffect(() => {
    setFilters(prev => ({
      ...prev,
      sport: [currentSport],
  ...
  const handleResetFilters = () => {
    setFilters({
      sortBy: 'recent',
      result: [],
      sport: [currentSport], // Reset to current sport
      team: [],
      tournament: [],
      dateRange: 'allTime',
    });
