-- RealVictors Database Schema Migration
...
CREATE TABLE public.profiles (
  ...
  primary_sports JSONB DEFAULT '[]', -- Array of sport strings
...
CREATE TABLE public.team_members (
  ...
  UNIQUE(team_id, jersey_number) -- Unique jersey numbers per team
);
...
CREATE TABLE public.games (
  ...
  -- Host information (either user or team, but not both)
  ...
  bet_pool_cents INTEGER DEFAULT 0 CHECK (bet_pool_cents >= 0), -- For exhibition betting
  ...
  -- Ensure either user or team host is specified
  CONSTRAINT host_required CHECK (
    (host_user_id IS NOT NULL AND host_team_id IS NULL) OR 
    (host_user_id IS NULL AND host_team_id IS NOT NULL)
  )
);
...
CREATE TABLE public.game_rsvps (
  ...
  paid_cents INTEGER DEFAULT 0 CHECK (paid_cents >= 0),
  payment_intent_id TEXT, -- Stripe payment intent
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  
  -- Ensure either user or team RSVP, but not both
  CONSTRAINT user_or_team_required CHECK (
    (user_id IS NOT NULL AND team_id IS NULL) OR 
    (user_id IS NULL AND team_id IS NOT NULL)
  ),
  -- Unique RSVP per game per user/team
...
CREATE TABLE public.player_stats (
  ...
  rank_local INTEGER CHECK (rank_local > 0),
  rank_global INTEGER CHECK (rank_global > 0),
...
CREATE TABLE public.game_events (
  ...
  game_minute INTEGER CHECK (game_minute >= 0), -- Game minute when event occurred
